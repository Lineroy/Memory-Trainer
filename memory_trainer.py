    ...
    try:
        how_len_number = int(input(
            "\nHow much significant will your number be?: " if language == 1
                    else "Сколько значное будет ваше число?: ")) - 1
...
while len(how_numbers) != how_len_number + 1:
    how_numbers += "0"
...
# Exit from settings;
                continue_or_again = input("Do You want to exit the settings?(Y, N): " if language == 1
                                    else "Вы хотите выйти с настроек?(Д, Н):").upper()

                if continue_or_again == "Y" or "Д":
                    continue
                elif continue_or_again == "N" or "Н":
                    break
...
# Conclusion of all answers: True or False;
    print(Style.BRIGHT + f"{'Answers' if language == 1 else 'Ответ'}: " + (Fore.GREEN + Style.BRIGHT + str(counter_True)), "/",
